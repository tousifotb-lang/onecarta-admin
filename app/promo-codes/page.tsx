...
import {
  Plus, X, Edit3, Trash2, ChevronLeft, ChevronRight, Loader2, Truck
} from "lucide-react";
...
export default function PromoCodeManagerMatrix() {
  ...
  const [freeDeliveryScope, setFreeDeliveryScope] = useState<"dhaka" | "all">("dhaka");
  ...
  const handleOpenEditModal = (item: PromoCode) => {
    ...
    setFreeDeliveryScope(item.freeDeliveryScope === "all" ? "all" : "dhaka");
                ...
                <div key={promo._id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-3xs flex flex-col min-h-64 justify-between transition-all hover:shadow-2xs">
                  ...
                  <div className="bg-[#42aa77] text-white p-4.5 space-y-4 relative flex-1 flex flex-col justify-between">
                    ...
                    <div className="space-y-2 text-[11px] font-bold text-emerald-50/90 tracking-wide">
                      ...
                      {promo.freeDelivery && (
                        <div className="flex flex-col">
                          <span className="text-[10px] text-white/60 font-medium uppercase tracking-wider flex items-center gap-1">
                            <Truck size={10} /> Free Delivery
                          </span>
                          <span className="text-xs font-semibold text-white mt-0.5">
                            {promo.freeDeliveryScope === "all" ? "All Areas" : "Inside Dhaka Only"}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              {discountType === "flat" ? (
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">
                    Flat Discount Amount (৳) {!freeDelivery && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="number"
                    required={!freeDelivery}
                    value={flatAmount}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="w-3.5 h-3.5 rounded-full border border-gray-300 text-gray-400 flex items-center justify-center text-[9px] font-black cursor-help" title="Waive the delivery charge entirely when this coupon is applied.">?</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setFreeDelivery(!freeDelivery)}
                    className={`w-8 h-4.5 rounded-full relative transition-colors duration-200 cursor-pointer ${
                      freeDelivery ? "bg-indigo-600" : "bg-slate-300"
                    }`}
                  >
                    <span className={`w-3.5 h-3.5 bg-white rounded-full absolute top-0.5 shadow-3xs transition-all duration-200 ${
                      freeDelivery ? "right-0.5" : "left-0.5"
                    }`} />
                  </button>
                </div>
                {freeDelivery && (
                ...
                <p className="text-[10px] text-gray-400 leading-relaxed font-medium">
                  When enabled, delivery charge becomes ৳0 for eligible orders — independent of any discount above.
                </p>
              </div>
